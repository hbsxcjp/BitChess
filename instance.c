...
static RecFormat getRecFormat(const char* ext)
{
    ...
    char tempExt[len];
    for (int i = 0; i <= len; ++i)
        tempExt[i] = tolower(ext[i]);

    for (RecFormat fmt = XQF; fmt <= JSON; ++fmt)
        if (strcmp(tempExt, EXTNAMES[fmt]) == 0)
            return fmt;
...
static unsigned char KeyXYf = 0, KeyXYt = 0, F32Keys[32] = { 0 };
...
static void __readMoveData(char* data, wchar_t* remark, FILE* fin)
{
    ...
    if (Version <= 10 || (data[2] & 0x20)) {
        ...
        RemarkSize = *(int32_t*)clen - KeyRMKSize;
    }
...
static void __readMove(Move* move, FILE* fin)
{
    ...
    assert(fcolrow < BOARDLENGTH && tcolrow < BOARDLENGTH);
    // wprintf(L"%3d: %d->%d\n", __LINE__, fcolrow, tcolrow);

    // *move->fseat = getSeat_rc(fcolrow % 10, fcolrow / 10);
    // *move->tseat = getSeat_rc(tcolrow % 10, tcolrow / 10);
    move->fromIndex = INDEXFROMROWCOL(fcolrow % 10, fcolrow / 10);
    move->toIndex = INDEXFROMROWCOL(tcolrow % 10, tcolrow / 10);
...
static void readXQF(Instance* ins, FILE* fin)
{
    ...
    // 取得棋子字符串
    char pieChars[BOARDLENGTH + 1] = { 0 };
    memset(pieChars, NullChar, BOARDLENGTH);
    const char QiziChars[] = "RNBAKABNRCCPPPPPrnbakabnrccppppp"; // QiziXY设定的棋子顺序
    for (int i = 0; i != pieceNum; ++i) {
        int xy = head_QiziXY[i];
        if (xy < BOARDLENGTH) // 用单字节坐标表示, 将字节变为十进制,  十位数为X(0-8),个位数为Y(0-9),棋盘的左下角为原点(0, 0)
            pieChars[xy % 10 * 9 + xy / 10] = QiziChars[i];
    }
    // setBoard(ins->board, pieChars);

    wchar_t tempStr[REMARKSIZE];
    swprintf(tempStr, REMARKSIZE, L"%d", Version);
    addInfoItem(ins, L"Version", tempStr); // 整数存储

    char fen[BOARDLENGTH];
    getFenFromPieChars(fen, pieChars);
    mbstowcs(tempStr, fen, BOARDLENGTH - 1);
    ...
    for (int i = 0; i != sizeof(names) / sizeof(names[0]); ++i) {
        char valueStr[REMARKSIZE + 1] = { 0 };
        size_t inlen = strlen(values[i]),
               outlen = REMARKSIZE;
        code_convert("gb2312", "utf-8", values[i], inlen, valueStr, outlen);
        mbstowcs(tempStr, valueStr, REMARKSIZE - 1);
    ...
    setRemark(ins->root, tempStr);
    // wprintf(L"%3d: %s\n", __LINE__, tempStr);

    if (data[2] & 0x80) // # 有左子树
        __readMove(addNext(ins->root), fin);
}
...
// static const wchar_t* getMoveInfo(wchar_t* str, size_t n, const Instance* ins)
// {
...
{
    Instance* ins = calloc(1, sizeof(Instance));
    ins->board = calloc(1, sizeof(Board));
    ins->root = newMove();
    ins->current = ins->root;
...
void addInfoItem(Instance* ins, const wchar_t* name, const wchar_t* value)
{
    ...
    if (count == 32 || name == NULL || value == NULL || wcslen(name) == 0 || wcslen(value) == 0)
        return;

    ins->info[count][0] = (wchar_t*)malloc((wcslen(name) + 1) * sizeof(name[0]));
    wcscpy(ins->info[count][0], name);

    ins->info[count][1] = (wchar_t*)malloc((wcslen(value) + 1) * sizeof(value[0]));
    wcscpy(ins->info[count][1], value);

    ++(ins->infoCount);
}

wchar_t* getInfoString(wchar_t* infoString, const Instance* ins)
{
    infoString[0] = L'\x0';
    wchar_t tempStr[REMARKSIZE];
    for (int i = 0; i < ins->infoCount; ++i) {
        swprintf(tempStr, REMARKSIZE, L"[%ls \"%ls\"]\n", ins->info[i][0], ins->info[i][1]);
...
void testInstance()
{
    ...
    wchar_t tempStr[MOVESTRSIZE];
    wprintf(L"testInstance:\n%ls\n", getInfoString(tempStr, ins));
    wprintf(L"%ls\n", getMovStringIccsZh(tempStr, MOVESTRSIZE, ins->root, PGN_ICCS));
